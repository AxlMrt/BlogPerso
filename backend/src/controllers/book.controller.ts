...
const createBook: RequestHandler = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!existingUser) next(new HttpException(400, "User doesn't exist."));

    if (!title || !author) next(new HttpException(400, 'Missing data'));

    if ((feedBack && !Number(feedBack)) || (year && !Number(year))) next(new HttpException(400, 'Invalid number'));

    if (feedBack < 0 || feedBack > 5) next(new HttpException(400, 'Feedback invalid'));
...
const updateBook: RequestHandler = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!bookToUpdate) next(new HttpException(404, 'Book not found'));
