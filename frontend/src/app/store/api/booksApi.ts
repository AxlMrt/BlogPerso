...
export const bookApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    deleteBook: builder.mutation<void, { bookId: string, id: string }>({
      query: ({ bookId, id }) => ({ url: `/books/${bookId}`, method: 'delete', body: id })
    }),
  }),
});

export const { useGetBooksQuery, useGetBookQuery, useAddNewBookMutation, useUpdateBookMutation, useDeleteBookMutation } = bookApi;